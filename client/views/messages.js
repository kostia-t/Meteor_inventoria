    ...
    swal({
      ...
      if(msg) {
        ...
        Meteor.call('messages.remove', msgId, whoDeleted, (error, result) => {
          if(error) {
            Bert.alert('Message couldn\'t be deleted.', 'danger', 'fixed-top');
          } else {
            Bert.alert('Message deleted', 'success', 'fixed-top');
          }
        });
      } else {
        Bert.alert('Message couldn\'t be deleted.', 'danger', 'fixed-top');
...
Template.singleMessage.onDestroyed(function () {
  $('.footer').show();
});
  ...
  'submit [data-id=reply-message-form]': (event, template) => {
    ...
    if(!$('[data-id=reply-submit]').hasClass('disabled')) {
      ...
      if(!body.toString().trim()) {
        fieldEmpty = true;
        Bert.alert('Please enter a message.', 'danger', 'fixed-top');
      }
      if(!fieldEmpty && !toUserId) {
        fieldEmpty = true;
        Bert.alert('Error sending message.', 'danger', 'fixed-top');
      }
      ...
      if(!fieldEmpty) {
        ...
        Meteor.call('messages.addMessage', Session.get('selectedMsg'), toUserId, body, (error, result) => {
          if(error) {
            ...
            Bert.alert('Message sent', 'success', 'fixed-top');
...
Template.compose.events({
  ...
  'submit [data-id=send-message-form]': (event, template) => {
    ...
    if(!$('[data-id=message-submit]').hasClass('disabled')) {
      ...
      if(!to) {
        fieldEmpty = true;
        Bert.alert('Please enter a username in the To field.', 'danger', 'fixed-top');
      }
      ...
      if(!fieldEmpty) {
        ...
        if(toUser) {
          ...
          if(toUser._id != Meteor.userId())
          {
            ...
            Bert.alert('Sorry, you can only send messages to other users.', 'danger', 'fixed-top');
          }
        } else {
          //Wasn't a valid user, display error
          Bert.alert('Sorry, we can\'t find that user.  Please verify the username and try again.', 'danger', 'fixed-top');
        }
      }
    }
  }
